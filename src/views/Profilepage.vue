...
// ── Enrich user with profile fields (mock, no backend) ──
const user = reactive({
  ...authUser.value,
...
function calcStrength() {
  ...
  const levels = [
    { pct: 25, label: 'Weak', color: '#FF5A5F' },
    { pct: 50, label: 'Fair', color: '#f4a261' },
    { pct: 75, label: 'Good', color: '#2EC4B6' },
    { pct: 100, label: 'Strong', color: '#06d6a0' },
  ]
  Object.assign(pwStrength, levels[score - 1] || { pct: 10, label: 'Too short', color: '#FF5A5F' })
}

const canChangePassword = computed(() =>
  pwForm.current && pwForm.newPw && pwForm.confirm && pwForm.newPw === pwForm.confirm
)

function changePassword() {
  pwError.value = null
  if (pwForm.current === 'wrongpassword') {
...
const notifGroups = [
  {
    key: 'bookings', title: 'Bookings & Trips',
    items: [
      { key: 'booking_confirm', label: 'Booking Confirmation' },
      { key: 'booking_reminder', label: 'Trip Reminders' },
      { key: 'booking_cancel', label: 'Cancellation Alerts' },
    ]
  },
  {
    key: 'messages', title: 'Messages',
    items: [
      { key: 'msg_new', label: 'New Messages' },
      { key: 'msg_reply', label: 'Message Replies' },
    ]
  },
  {
    key: 'promo', title: 'Promotions',
    items: [
      { key: 'promo_deals', label: 'Special Deals' },
      { key: 'promo_news', label: 'Voyago News & Updates' },
    ]
  },
]
...
const notifSettings = reactive(
  Object.fromEntries(
    notifGroups.flatMap(g => g.items).map(i => [i.key, { email: true, push: true, sms: false }])
  )
)

function saveNotifs() {
  savedMessage.value = 'notifs'
  setTimeout(() => savedMessage.value = null, 3000)
}
...
function deactivateAccount() {
  alert('Account deactivated. (Mock — no backend)')
}
...
function requestVerification() {
  alert('Verification request submitted! Our team will contact you within 2–3 business days. (Mock)')
}
</script>
...
.avatar-upload-btn:hover { background: var(--coral-dk, #e04045); }

.sidebar-name {
  font-family: 'Fraunces', serif;
  font-size: 20px;
  color: var(--indigo, #2D3142);
  margin: 0 0 6px;
}
...
.role-agency  { background: var(--coral-lt, rgba(255,90,95,.10)); color: var(--coral); }
...
.sidebar-nav-btn:hover { background: var(--gray-100, #F2F2F2); color: var(--indigo); }
.sidebar-nav-btn.active { background: var(--teal-lt, rgba(46,196,182,.10)); color: var(--teal, #2EC4B6); font-weight: 600; }
.sidebar-nav-btn.active .nav-icon { color: var(--teal); }
...
.verify-icon.verified { background: rgba(46,196,182,.12); color: var(--teal); }
.verify-icon.unverified { background: rgba(255,90,95,.10); color: var(--coral); }
