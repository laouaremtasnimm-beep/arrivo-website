...
const API_BOOKINGS = 'http://localhost/arrivo-website/backend/api/v1/bookings.php'
...
async function fetchBookings() {
  ...
  const url =
    role.value === 'agency'   ? `${API_BOOKINGS}?agency_id=${uid}` :
    role.value === 'provider' ? `${API_BOOKINGS}?provider_id=${uid}` :
                                `${API_BOOKINGS}?user_id=${uid}`
  try {
    const res  = await fetch(url, { cache: 'no-store' })
    const data = await res.json()
    if (res.ok) bookings.value = data.bookings ?? []
  } catch (e) {
...
async function handleConfirm(b) { await updateStatus(b.id, 'confirmed'); b.status = 'confirmed' }
async function handleCancel(b)  { await updateStatus(b.id, 'cancelled'); b.status = 'cancelled' }
function handleView(b) { console.log('View booking', b) }
...
.bk-hero {
  ...
  padding-top: calc(72px + 72px); /* navbar height */
  overflow: hidden;
}
...
.bk-hero__blob {
  ...
  background: radial-gradient(circle, rgba(255,90,95,.18) 0%, transparent 70%);
  pointer-events: none;
}
...
.bk-tabs-wrap {
  ...
  padding: 0 var(--section-px, 6%); box-shadow: 0 2px 12px rgba(45,49,66,.05);
  flex-wrap: wrap; gap: 4px;
}
.bk-tabs { display: flex; align-items: center; gap: 4px; }
.bk-tab {
  display: flex; align-items: center; gap: 8px;
  padding: 18px 20px 16px; background: none; border: none;
...
.bk-search__input {
  ...
  color: var(--indigo); background: var(--white); outline: none;
...
.bk-card__thumb {
  ...
  background: linear-gradient(135deg, var(--sand, #f5ede0) 0%, var(--gray-100) 100%);
  display: flex; align-items: center; justify-content: center;
}
.bk-card__thumb-bg { font-size: 3rem; }
.bk-card__type-tag {
  position: absolute; top: 12px; left: 12px;
  background: rgba(45,49,66,.7); backdrop-filter: blur(6px);
...
.bk-card__title {
  ...
  font-weight: 700; color: var(--indigo); margin: 0; line-height: 1.3;
}
...
/* Shared button overrides for small size */
